...
namespace VolvoRefactor.IntegrationTests.MockedProviders
{
    public class TestTaxConfigurationProvider : ITaxConfigurationProvider
    {
        public async Task<CongestionConfiguration> ImportConfigurationFromFile(string city)
        {
            return await Task.FromResult(new CongestionConfiguration
            {
                City = "TestCity",
                MaxDailyFee = 60,
                SingleChargeTime = 60,
